...
public class EventServiceImpl implements EventFacade {
	...
	public List<Event> getAllEvents() {
		...
		return events; // This return value will vary depending on the coding based on Todos above. It may be ArrayList<Event[]>  
	}
	...
	public boolean checkEventsofVisitor(Visitor visitor, int eventid) {
		...
		boolean eventsOfVisitorExists =false  ; 
		EventDAO eventDAO = new EventDAO() ; 
		try {
			eventsOfVisitorExists = eventDAO.checkEventsofVisitor(visitor, eventid);
		} catch (ClassNotFoundException e) {
			log.info(e.getMessage());
		} catch (SQLException e) {
			log.info(e.getMessage());
		...
		return eventsOfVisitorExists; // replace "false" with the "status" variable used in the code in 'TODO' section
	}
